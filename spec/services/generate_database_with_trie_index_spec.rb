...
describe Services::GenerateDatabase do
  describe '#call' do
    ...
    context 'when users data is provided' do
      ...
      it 'generates database with users 1 and 2 data populated' do
      ...
      it 'generates database index for the url attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'url')
        ).to match({
      ...
      it 'generates database index for the external_id attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'external_id')
        ).to match({
      ...
      it 'generates database index for the name attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'name')
        ).to match({
      ...
      it 'generates database index for the alias attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'alias')
        ).to match({
                     'miss coffey' => [1], '' => [2]
                   })
      end
      ...
      it 'generates database index for the created_at attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'created_at')
        ).to match({
                     2016 => { 4 => { 15 => { 15 => { 19 => { 46 => [
                       1, 2
                     ] } } } } }
      ...
      it 'generates database index for the active attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'active')
        ).to match({
                     true => [1], false => [2]
                   })
      end
      ...
      it 'generates database index for the verified attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'verified')
        ).to match({
                     true => [1], false => [2]
                   })
      end
      ...
      it 'generates database index for the shared attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'shared')
        ).to match({
                     true => [1], '' => [2]
                   })
      end
      ...
      it 'generates database index for the locale attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'locale')
        ).to match({
                     'en-au' => [1], '' => [2]
                   })
      end
      ...
      it 'generates database index for the timezone attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'timezone')
        ).to match({
                     'sri lanka' => [1], '' => [2]
                   })
      end

      it 'generates database trie index for the last_login_at attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'last_login_at')
        ).to match({
                     2013 => { 8 => { 4 => { 11 => { 3 => { 27 => [1] } } } } },
                     '' => [2]
                   })
      ...
      it 'generates database index for the email attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'email')
        ).to match({
      ...
      it 'generates database index for the phone attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'phone')
        ).to match({
                     '8335-422-718' => [1], '' => [2]
                   })
      end
      ...
      it 'generates database index for the signature attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'signature')
        ).to match({
      ...
      it 'generates database index for the organization_id attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'organization_id')
        ).to match({
                     101 => [1],
                     '' => [2]
                   })
      ...
      it 'generates database index for the tags attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'tags')
        ).to match({
                     'springville' => [1],
                     'sutton' => [1, 2],
                     'melbourne' => [2]
      ...
      it 'generates database index for the suspended attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'suspended')
        ).to match({
                     false => [1], '' => [2]
                   })
      end
      ...
      it 'generates database index for the role attribute' do
        expect(
          database.value!.data.dig('users', 'index', 'role')
        ).to match({
                     'admin' => [1], '' => [2]
                   })
      end
    ...
    context 'when organizations data is provided' do
      it 'generates database 2 organizations data populated' do
        expect(
          database.value!.data.dig('organizations')
          .except('index').size
        ).to eq(2)
      end
      ...
      it 'generates database with organization 1 data populated' do
        expect(
          database.value!.data.dig('organizations', 101)
        ).to match(organization_json[0])
      end

      it 'generates database with organization 2 data populated' do
        expect(
          database.value!.data.dig('organizations', 102)
        ).to match(organization_json[1])
      end

      it 'generates database index for the url attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'url')
        ).to match({
      ...
      it 'generates database index for the external_id attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'external_id')
        ).to match({
      ...
      it 'generates database index for the domain_names attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'domain_names')
        ).to match({
                     'kage.com' => [101],
                     'zentix.com' => [101,
                                      102],
      ...
      it 'generates database index for the created_at attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'created_at')
        ).to match({
                     2016 => { 5 => { 21 => { 21 => { 10 => { 28 => [
                       101, 102
                     ] } } } } }
      ...
      it 'generates database index for the tags attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'tags')
        ).to match({
                     'fulton' => [101, 102],
                     'farley' => [101],
                     'barley' => [102]
      ...
      it 'generates database index for the shared_tickets attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'shared_tickets')
        ).to match({
                     false => [101], '' => [102]
                   })
      end
      ...
      it 'generates database index for the details attribute' do
        expect(
          database.value!.data.dig('organizations', 'index', 'details')
        ).to match({
                     'megacorp' => [101],
                     '' => [102]
                   })
    ...
    context 'when tickets data is provided' do
      ...
      it 'generates database index for the url attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'url')
        ).to match(
          {
      ...
      it 'generates database index for the external_id attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'external_id')
        ).to match({
      ...
      it 'generates database index for the created_at attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'created_at')
        ).to match({
                     2016 => { 4 => { 28 => { 21 => { 19 => { 34 => [
                       ticket_1_id, ticket_2_id
                     ] } } } } }
                   })
      end
      ...
      it 'generates database index for the due_at attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'due_at')
        ).to match({
      ...
      it 'generates database index for the tags attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'tags')
        ).to match({
                     'ohio' => [ticket_1_id],
      ...
      it 'generates database index for the has_incidents attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'has_incidents')
        ).to match({
                     false => [ticket_1_id,
                               ticket_2_id]
                   })
      ...
      it 'generates database index for the type attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'type')
        ).to match({
      ...
      it 'generates database index for the subject attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'subject')
        ).to match({
      ...
      it 'generates database index for the description attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'description')
        ).to match(
          {
            'ladida' => [ticket_1_id],
            '' => [ticket_2_id]
      ...
      it 'generates database index for the priority attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'priority')
        ).to match({
                     'high' => [ticket_1_id],
                     '' => [ticket_2_id]
                   })
      ...
      it 'generates database index for the status attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'status')
        ).to match({
                     'pending' => [ticket_1_id],
                     '' => [ticket_2_id]
                   })
      ...
      it 'generates database index for the via attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'via')
        ).to match({
                     'web' => [ticket_1_id],
                     '' => [ticket_2_id]
                   })
      ...
      it 'generates database index for the submitter_id attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'submitter_id')
        ).to match({
                     1 => [ticket_1_id],
                     '' => [ticket_2_id]
                   })
      ...
      it 'generates database index for the assignee_id attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'assignee_id')
        ).to match({
                     2 => [ticket_1_id],
                     '' => [ticket_2_id]
                   })
      ...
      it 'generates database index for the organization_id attribute' do
        expect(
          database.value!.data.dig('tickets', 'index', 'organization_id')
        ).to match({
                     101 => [ticket_1_id],
                     '' => [ticket_2_id]
                   })
